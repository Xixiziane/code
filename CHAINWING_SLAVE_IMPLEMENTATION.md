...
| 从机位置调整执行器 | **升降副翼(elevon)** — elevon偏转产生气动力矩修正铰链相对滚转角 |
...
主从机各有独立的IMU。铰链角 = 从机滚转角 - 主机滚转角（铰链轴=X轴=滚转轴）。

在硬件上：
1. 主机通过MAVLink发送自身滚转角/角速度
2. 从机读取自身IMU数据
3. 差值 = 铰链相对角度
...
从机通过升降副翼（elevon）产生气动力矩来修正铰链相对滚转角：
...
其中:
  δ_master = 主飞控发送的整体命令 (通过 MAVLink)
...
| ATTITUDE | 50 Hz | 主机滚转角/角速度（IMU积分参考，铰链轴=滚转轴） |
...
### 25.10 第二次修改：基于 report.pdf 重新设计（2026-03-22）

**问题**: §25.8 方案 A（k=50, c=5）在仿真中飞行不稳定。

**根本原因分析**:
- k=50 时自然频率 ωn=8.55 rad/s (1.36 Hz)
- PX4 固定翼姿态控制器带宽 ~1-2 Hz
- **铰链结构模态与飞行控制器频率耦合** → 不稳定振荡

**基于 report.pdf 的重新设计**:

report.pdf §3.2.1 描述：
- 翼尖连接采用 "刚性核心-弹性缓冲" 三明治结构
- 中间层：薄型高强度刚性板（轻质合金）
- 上下层：厚壁弹性元件（**高弹性模量聚氨酯**）
- 允许 **±45°** 滚转自由度
- MATLAB 模型（ThreeBodyDynamics.m）使用理想球铰 (k=0, c=0)

**缩放推导（prototype → simulation）**:

| 参数 | report.pdf 原型 | GZ 仿真 | 缩放比 |
|------|----------------|---------|--------|
| 单机质量 | 0.5 kg | 1.9 kg | ×3.8 |
| 翼展 | 0.6 m | 1.2 m | ×2.0 |
| 巡航速度 | 15 m/s | 20 m/s | ×1.33 |
| 翼面积 | 0.108 m² | 0.36 m² | ×3.33 |
| 铰链惯量 | 0.045 kg·m² | 0.684 kg·m² | ×15.2 |

聚氨酯刚度估算 → 频率匹配缩放 → 安全系数修正:
```
聚氨酯垫 (50×50×5mm, E=30MPa, 上下双层):
  I_section = 0.05×0.05³/12 = 5.21e-7 m⁴
  k_pad = E×I/L = 30e6 × 5.21e-7 / 0.005 = 3.13 N·m/rad
  k_real = 2 × k_pad = 6.3 N·m/rad

频率匹配缩放至仿真尺度:
  ωn_real = √(6.3/0.045) = 11.8 rad/s
  k_base = ωn² × I_sim = 11.8² × 0.684 = 95.3 N·m/rad
  k_sim = k_base × 2.1 = 200 N·m/rad
  (×2.1 = 动压比 245/138=1.78 × 18% 数值裕量)
  c_sim = 2ζ√(kI) = 2×0.51×√(200×0.684) = 12 N·m·s/rad
```
(详细推导见 CHAINWING_PARAMETER_REFERENCE.md §3.3)

**参数修改对比（三次迭代）**:

| 参数 | 初始值 | 第一次修改 (§25.8) | **第二次修改** |
|------|--------|-------------------|---------------|
| 弹簧刚度 k | 500 | 50 | **200** |
| 阻尼系数 c | 50 | 5 | **12** |
| 角度限制 | ±5° | ±10° | **±15°** |
| PD Kp | 0.3 | 2.0 | **1.5** |
| PD Kd | 0.05 | 0.3 | **0.2** |
| ωn (Hz) | 11.9 | 1.36 | **2.72** |
| ζ | 3.74 | 1.18 | **0.51** |
| 控制器占比 | 4.9% | 77% | **38%** |
| 飞行效果 | ✅ 稳定但控制无效 | ❌ 不稳定 | ⏳ 待验证 |

**设计理念变化**:
- 第一次修改: "让控制器主导" → k太小 → 结构不稳定
- 第二次修改: "弹簧保基础稳定, 控制器做精细调整" → k=200, 控制器38%
